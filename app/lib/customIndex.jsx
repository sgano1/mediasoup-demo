...
import RoomClient from './CustomRoomClient';
...
async function run() {
	...
	const peerId = appConfig.peerId || randomString({ length: 8 }).toLowerCase();
	let roomId = appConfig.roomId || urlParser.query.roomId;
	let displayName =
		appConfig.displayName || urlParser.query.displayName || (cookiesManager.getUser() || {}).displayName;
	const handlerName = appConfig.handlerName || urlParser.query.handlerName || urlParser.query.handler;
	const forceTcp = urlParser.query.forceTcp === 'true' || !!appConfig.forceTcp;
	const produce = !(urlParser.query.produce === 'false' || appConfig.produce === false);
	const consume = !(urlParser.query.consume === 'false' || appConfig.consume === false);
	const datachannel = !(urlParser.query.datachannel === 'false' || appConfig.datachannel === false);
	const forceVP8 = urlParser.query.forceVP8 === 'true' || appConfig.forceVP8 === true;
	const forceH264 = urlParser.query.forceH264 === 'true' || appConfig.forceH264 == true;
	const forceVP9 = urlParser.query.forceVP9 === 'true' || appConfig.forceVP9 == true;
	const enableWebcamLayers = !(urlParser.query.enableWebcamLayers === 'false' || appConfig.enableWebcamLayers === false);
	const enableSharingLayers = !(urlParser.query.enableSharingLayers === 'false' || appConfig.enableSharingLayers === false);
	...
	const faceDetection = urlParser.query.faceDetection === 'true' || appConfig.faceDetection === true;
	const externalVideo = urlParser.query.externalVideo === 'true' || appConfig.externalVideo === true;
	...
	render(
		<div className="ui grid">
			...
			<div className="fourteen wide column">
				<BoardComponent {...appConfig.boardConfig} />
			</div>
		</div>
		,
		appConfig.container || document.getElementById('mediasoup-demo-app-container')
	);
}
...
window.mediasoupApp = mediasoupApp;